...
class CloudScanner:
    ...
    async def scan_aws(self, account_id: str = "", region: str = "") -> CloudScanResult:
        ...
        def _make_default(cid: str, status: str, details: str, rec: str, ev: str) -> dict[str, Any]:
            return {
                "check_name": cid,
                "status": status,
                "severity": "medium",
        ...
        for check_def in _CLOUD_CHECKS.get("aws", []):
            ...
            if cid == "S3_PUBLIC_ACCESS":
                if boto3_available and aws_account_id:
                    f.update(_make_default(cid, "error",
                    ...
                    f.update(_make_default(cid, "fail",
    ...
    async def scan_azure(self, subscription_id: str = "") -> CloudScanResult:
        ...
        return self._build_result(CloudProvider.AZURE, target or "azure-unknown", findings)
    ...
    async def scan_gcp(self, project_id: str = "") -> CloudScanResult:
        ...
        gcp_project_from_env = os.environ.get("GCP_PROJECT", os.environ.get("GOOGLE_CLOUD_PROJECT", ""))
    ...
    async def scan_by_provider(self, provider: CloudProvider, target: str = "") -> CloudScanResult:
        ...
        if not scanner:
            return CloudScanResult(
                provider=provider, target=target, error=f"Unsupported provider: {provider}"
            )
        return await scanner(target)

    def _build_result(self, provider: CloudProvider, target: str, findings: list[dict[str, Any]]) -> CloudScanResult:
        import time
        import uuid
