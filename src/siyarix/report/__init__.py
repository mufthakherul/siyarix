...
class ReportEngine:
    ...
    def build_report(
        ...
        report.metadata = {
            ...
            "generated_by": "Siyarix Report Engine v1.0.1",
        }
...
- Exploitation verification: sqlmap, hydra (as applicable)"""
    ...
    def _render_html(self, report: Report) -> str:
        ...
        html = [
            ...
            ""
            if not getattr(report.config, "offline", False)
            else "<link href='https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;800&family=Fira+Code&display=swap' rel='stylesheet'>",
            "<style>"
            + (
    ...
    def _render_sarif(self, report: Report) -> str:
        ...
        sarif_output = {
            ...
            "runs": [
                {
                    "tool": {"driver": {"name": "Siyarix", "version": "1.0.1"}},
                    "results": results,
                    "properties": {
                        "report_id": report.report_id,
                        "generated_at": report.generated_at,
