...
_BUILTIN_COMMANDS: list[CommandInfo] = [
    ...
    CommandInfo(
        ...
        usage="/skills [stats|list|show|edit|remove|add|export|run|search|import]",
        handler="_cmd_skills",
        examples=[
            "/skills list",
            "/skills show 3",
            "/skills run 1 example.com",
            "/skills search nmap",
    ...
    CommandInfo(
        name="/memory",
        category=CommandCategory.SESSION,
        description="Query and manage persistent long-term memory and target intelligence",
        usage="/memory [list|search|target|stats|clear] [args...]",
        handler="_cmd_memory",
        examples=[
            "/memory list",
            "/memory list persistent",
            "/memory search port 80",
            "/memory target example.com",
            "/memory stats",
            "/memory clear",
        ],
        notes="Inspects or searches SQLite long-term memory across targets, findings, and sessions.",
    ),
    CommandInfo(
        name="/feedback",
        category=CommandCategory.LEARNING,
    ...
    CommandInfo(
        ...
        description="Toggle split pane working window",
        usage="/split [subagents|tasks|findings|logs|timeline|metrics|cheatsheet|attack_map|off]",
        handler="_cmd_split",
        examples=[
            "/split subagents",
            "/split tasks",
        ...
        notes="Displays responsive side-by-side working window. Use Alt+W or Alt+S to cycle views.",
    ),
    CommandInfo(
        name="/batch",
        category=CommandCategory.ADVANCED,
    ...
    CommandInfo(
        name="/agent",
        category=CommandCategory.ADVANCED,
        description="Subagent fleet orchestration & execution preview",
        usage="/agent [list|run <goal>|spawn <role> <goal>|switch <id>|preview [id]|logs <id>|status|kill <id>|clear]",
        handler="_cmd_agent",
        examples=[
            "/agent list",
            "/agent run enumerate open ports on 10.0.0.1",
            "/agent spawn recon 'subdomain enum'",
            "/agent switch recon-1",
            "/agent preview recon-1",
            "/agent logs recon-1",
        ],
        notes="Spawns, previews, and manages specialized subagents. Use Alt+A or Shift+Tab to cycle focus.",
    ),
    CommandInfo(
        name="/siem",
        category=CommandCategory.ADVANCED,
