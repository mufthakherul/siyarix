...
class ExecutionMetrics:
    ...
    def to_metrics(self) -> list[Metric]:
        ...
        return [
            Metric("nexsec_scans_total", MetricType.COUNTER, self.total_scans),
            Metric("nexsec_scans_successful", MetricType.GAUGE, self.successful_scans),
            Metric("nexsec_scans_failed", MetricType.GAUGE, self.failed_scans),
            Metric("nexsec_findings_total", MetricType.GAUGE, self.total_findings),
            Metric("nexsec_execution_duration_seconds", MetricType.GAUGE, self.total_duration_seconds),
            Metric("nexsec_execution_avg_duration_seconds", MetricType.GAUGE, self.average_duration_seconds),
        ]
...
class ToolMetrics:
    ...
    def to_metrics(self) -> list[Metric]:
        ...
        return [
            Metric(
                "nexsec_tool_executions",
                MetricType.GAUGE,
                self.executions,
                {"tool": self.tool_name},
            ),
            Metric(
                "nexsec_tool_successful",
                MetricType.GAUGE,
                self.successful,
                {"tool": self.tool_name},
            ),
...
class PlannerMetrics:
    ...
    def to_metrics(self) -> list[Metric]:
        """Convert to Prometheus metrics."""
        return [
            Metric("nexsec_plans_generated", MetricType.COUNTER, self.plans_generated),
            Metric("nexsec_plans_successful", MetricType.GAUGE, self.plans_successful),
            Metric("nexsec_plans_failed", MetricType.GAUGE, self.plans_failed),
            Metric("nexsec_model_calls", MetricType.COUNTER, self.model_calls),
            Metric("nexsec_model_errors", MetricType.COUNTER, self.model_errors),
            Metric("nexsec_interpreter_fallbacks", MetricType.COUNTER, self.interpreter_fallbacks),
        ]
...
class MetricsCollector:
    ...
    def to_prometheus(self) -> str:
        ...
        lines = [
            "# HELP nexsec_uptime Uptime in seconds",
            "# TYPE nexsec_uptime gauge",
            f"nexsec_uptime {self.uptime_seconds()}",
            "",
        ]
