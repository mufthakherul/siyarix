...
class SplunkHECConnector(SIEMConnector):
    ...
    def __init__(self, endpoint: str, token: str) -> None:
        if not HAS_HTTPX:
            raise ImportError(
                "httpx is required for SplunkHECConnector. Install with: pip install phalanx[siem]"
            )
...
class ElasticSIEMConnector(SIEMConnector):
    ...
    def __init__(self, endpoint: str, api_key: str, index: str = "phalanx-audit") -> None:
        if not HAS_HTTPX:
            raise ImportError(
                "httpx is required for ElasticSIEMConnector. Install with: pip install phalanx[siem]"
            )
...
class TelemetryForwarder:
    ...
    def _load_from_env(self) -> None:
        ...
        splunk_url = os.getenv("PHALANX_SPLUNK_URL")
        splunk_token = os.getenv("PHALANX_SPLUNK_TOKEN")
        if splunk_url and splunk_token:
            self.connectors.append(SplunkHECConnector(splunk_url, splunk_token))

        elastic_url = os.getenv("PHALANX_ELASTIC_URL")
        elastic_key = os.getenv("PHALANX_ELASTIC_KEY")
