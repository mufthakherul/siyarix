...
class AutonomousExecutor(BaseExecutor):
    ...
    Unlike RegistryExecutor, this does not use the ToolRegistry — commands
    are self-contained shell commands generated by the LLM.
    """
    ...
    async def _execute_tool_step(self, step: PlanStep) -> dict[str, Any]:
        ...
        return {"status": "error", "error": f"No handler for tool: {step.tool}"}
    ...
    async def _execute_shell_command(self, step: PlanStep, state: CommandResult) -> dict[str, Any]:
        ...
        cmd_timeout = step.timeout or 300.0
