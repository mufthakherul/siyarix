...
_CA_INFO_RE = re.compile(r"(?:CA\s+Name|Certificate\s+Authority|CA\s+Server)[:\s]+(.+)", re.IGNORECASE)
_PKCS12_RE = re.compile(r"(?:Saved\s+)?(?:PKCS12|\.p12|certificate|\.pfx)\s*(?:to|file)?[:\s]*(\S+)", re.IGNORECASE)
