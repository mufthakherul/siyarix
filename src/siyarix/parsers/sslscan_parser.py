...
_CERT_RE = re.compile(r"(?:Certificate|Subject|Issuer|Not valid|SHA-1|SHA-256|MD5)[:\s]+(.+)", re.IGNORECASE)
_PROTOCOL_RE = re.compile(r"(TLSv1\.\d|SSLv[23]|SSLv2|SSLv3)\s*:?\s+(supported|disabled|enabled)", re.IGNORECASE)
