    ...
    async def handler(**kwargs: Any) -> dict[str, Any]:
        ...
        if action == "shortest_path":
            ...
            if source and target:
                ...
                result = (
                    {"path": [n for n in path_nodes]} if path_nodes else {"error": "No path found"}
                )
            ...
            result = {
                "error": f"Unknown action: {action}. Supported: shortest_path, blast_radius, find_crown_jewel_paths"
            }
...
def make_threat_intel_handler() -> ToolHandler:
    async def handler(**kwargs: Any) -> dict[str, Any]:
        ...
        if action == "cve_lookup":
            ...
            data: dict[str, Any] = getattr(feed, "query_cve", lambda x: {})(
                query
            )  # Using getattr in case methods aren't exactly named this
            if not data:
