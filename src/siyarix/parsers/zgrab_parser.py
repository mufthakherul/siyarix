...
class ZgrabParser:
    ...
    def parse(self, output: str) -> list[dict]:
        ...
        for line in output.splitlines():
            ...
            for protocol in ("http", "https", "tls", "ssh", "ftp", "smtp", "pop3", "imap", "modbus", "bacnet"):
                proto_data = inner.get(protocol, data.get(f"{protocol}_response", {}).get("result", {}))
                if isinstance(proto_data, dict) and (proto_data.get("status", "") == "success" or bool(proto_data)):
                    findings.extend(self._parse_protocol(protocol, proto_data, ip, domain, timestamp, seen))
    ...
    def _parse_protocol(self, protocol: str, data: dict, ip: str, domain: str, timestamp: str, seen: set[str]) -> list[dict]:
        r: list[dict] = []
        if protocol == "tls":
            ...
            if cipher:
                ...
                if dedup_key not in seen:
                    ...
                    r.append({
                        ...
                        "severity": "low" if cipher in ("TLS_RSA_WITH_RC4_128", "TLS_RSA_WITH_3DES_EDE_CBC") else "info",
            ...
            r.append({
                "title": f"HTTP: {ip}",
                "severity": "info",
                "description": f"zgrab scanned HTTP on {ip}" + (f" (domain: {domain})" if domain else ""),
