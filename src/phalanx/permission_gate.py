...
class PermissionGate:
    ...
    def check(self, command: str, tool: str = "", context: dict[str, Any] | None = None) -> GateResult:
        ...
        for pattern in self._forbidden:
            if pattern.search(command):
                return GateResult(False, "forbidden", f"Contains forbidden pattern: {pattern.pattern}", tool=tool, command=command)
        ...
        if self._persona_engine and tool:
            ...
            if persona and hasattr(persona, 'tool_acl'):
                ...
                if not acl.is_allowed(tool):
                    return GateResult(False, "permission", f"Tool '{tool}' not in allowed list", tool=tool, command=command)
                if acl.requires_review(tool):
                    return GateResult(True, "review", f"Tool '{tool}' requires review", tool=tool, command=command, requires_review=True)
                if acl.requires_permission(tool):
                    return GateResult(True, "permission", f"Tool '{tool}' requires permission", tool=tool, command=command, requires_review=True)
