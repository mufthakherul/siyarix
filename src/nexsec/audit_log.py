...
class AuditLogger:
    ...
    def _save_events(self):
        ...
        self._AUDIT_DB.write_text(json.dumps(data, indent=2))

    def _startup_event(self):
        """Log system startup"""
        self.log(
            event_type=AuditEventType.SYSTEM_ERROR,  # Using as startup
            severity=AuditSeverity.INFO,
            user="system",
            action="cli_startup",
            result="success",
            details={"version": "2.0.0-PRO", "platform": sys.platform},
        )
    ...
    def end_session(self, session_id: str):
        ...
        if session_id in self._sessions:
            ...
            self.log(
                ...
                action="session_end",
                result="success",
                details={"duration": str(datetime.now() - self._sessions[session_id].start_time)},
            )
    ...
    def export(
        ...
        if format == "json":
            ...
            output = StringIO()
            writer = csv.DictWriter(output, fieldnames=["timestamp", "user", "action", "target", "result"])
            writer.writeheader()
            for e in events:
                writer.writerow({
