"""Phalanx CLI — CA-2.2 Real-Time Scan Progress.
...
class ScanProgressDisplay:
    ...
    def _render(self) -> Group:
        ...
        header = Text(
            f"Phalanx Scan  ⏱ {m:02d}:{s:02d}  Findings: {self._state.total_findings}",
            style="bold",
        )
        items = [
            Panel(
    ...
    def print_summary(self, target: str) -> None:
        ...
        if total == 0:
            ...
            lines.append("[dim]phalanx-agent history list  — view full results[/dim]")
