...
class ContinuousLearning:
    ...
    async def record_experience(self, exp: Experience) -> None:
        ...
        logger.info(f"Recorded learning experience for action: {exp.action}")

    async def query_similar_experiences(self, current_action: str, target: str, limit: int = 3) -> list[Experience]:
            ...
            try:
                ...
                if "embedding" in data and data["embedding"]:
                    ...
                    if score > 0.5: # Threshold
                        scored_exps.append((score, Experience(
                            target=data["target"],
                            action=data["action"],
                            result=data["result"],
