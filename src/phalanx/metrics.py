...
class ExecutionMetrics:
    ...
    def to_metrics(self) -> list[Metric]:
        ...
        return [
            Metric("phalanx_scans_total", MetricType.COUNTER, self.total_scans),
            Metric("phalanx_scans_successful", MetricType.GAUGE, self.successful_scans),
            Metric("phalanx_scans_failed", MetricType.GAUGE, self.failed_scans),
            Metric("phalanx_findings_total", MetricType.GAUGE, self.total_findings),
            Metric("phalanx_execution_duration_seconds", MetricType.GAUGE, self.total_duration_seconds),
            Metric("phalanx_execution_avg_duration_seconds", MetricType.GAUGE, self.average_duration_seconds),
        ]
...
class ToolMetrics:
    ...
    def to_metrics(self) -> list[Metric]:
        ...
        return [
            Metric(
                "phalanx_tool_executions",
                MetricType.GAUGE,
                self.executions,
                {"tool": self.tool_name},
            ),
            Metric(
                "phalanx_tool_successful",
                MetricType.GAUGE,
                self.successful,
                {"tool": self.tool_name},
            ),
...
class PlannerMetrics:
    ...
    def to_metrics(self) -> list[Metric]:
        """Convert to Prometheus metrics."""
        return [
            Metric("phalanx_plans_generated", MetricType.COUNTER, self.plans_generated),
            Metric("phalanx_plans_successful", MetricType.GAUGE, self.plans_successful),
            Metric("phalanx_plans_failed", MetricType.GAUGE, self.plans_failed),
            Metric("phalanx_model_calls", MetricType.COUNTER, self.model_calls),
            Metric("phalanx_model_errors", MetricType.COUNTER, self.model_errors),
            Metric("phalanx_interpreter_fallbacks", MetricType.COUNTER, self.interpreter_fallbacks),
        ]
...
class MetricsCollector:
    ...
    def to_prometheus(self) -> str:
        ...
        lines = [
            "# HELP phalanx_uptime Uptime in seconds",
            "# TYPE phalanx_uptime gauge",
            f"phalanx_uptime {self.uptime_seconds()}",
            "",
        ]
