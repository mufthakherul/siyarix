...
_DEFAULT_ROOT = Path(os.getenv("PHALANX_PLUGINS_DIR", str(Path.home() / ".phalanx" / "plugins")))
    ...
    def create_scaffold(self, name: str, author: str = "Unknown") -> Path:
        ...
        _write_plugin_yaml(
            ...
            PluginMetadata(
                ...
                version="0.1.0",
                author=author,
                description="Custom Phalanx plugin",
                path=str(plugin_dir),
            ),
        )
    ...
    def load_command_plugins(self, app: object | None) -> list[str]:
        ...
        for plugin in self.list_plugins():
            ...
            if not module_file.exists():
                continue
            try:
                module = self._load_module(module_file, f"phalanx_plugin_{plugin.name}_commands")
            except Exception:
    ...
    def load_parser_plugins(self) -> dict[str, Callable[[str], list[dict]]]:
        ...
        for plugin in self.list_plugins():
            ...
            if not module_file.exists():
                continue
            try:
                module = self._load_module(module_file, f"phalanx_plugin_{plugin.name}_parser")
            except Exception:
                logging.getLogger(__name__).exception(
