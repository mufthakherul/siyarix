...
from phalanx.security.attack_path import AttackPathAnalyzer
...
class ReportConfig:
    ...
    title: str = "Phalanx Security Assessment Report"
    author: str = "Phalanx Automated Agent"
    company: str = ""
    logo_url: str = ""
...
- Exploitation verification: sqlmap, hydra (as applicable)"""
    ...
    def _render_sarif(self, report: Report) -> str:
        ...
        for i, f in enumerate(report.findings):
            cvss = f.get("cvss", {})
            result = {
                "ruleId": f.get("tool", "phalanx") + "-" + str(i),
...
class ReportGenerator:
    """Generates premium executive and technical reports from KnowledgeGraph."""
    ...
    def generate_markdown(self) -> str:
        ...
        # 1. Header
        md = []
        md.append("# Phalanx Premium Executive Security Report")
...
<head>
    ...
    <title>Phalanx Security Report</title>
    <style>
        body {{
...
<body>
    ...
    def save_report(self, format: str = "markdown", path: Path | None = None) -> Path:
        ...
        if path is None:
            ...
            path = Path(f"phalanx_report_{timestamp}.{ext}")
