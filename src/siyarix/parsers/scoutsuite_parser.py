...
class ScoutsuiteParser:
    ...
    def parse(self, output: str) -> list[dict]:
        ...
        aws_account_id = data.get("aws_account_id", data.get("azure_subscription_id", data.get("gcp_project_id", "unknown")))
        provider = "aws" if "aws_account_id" in data else "azure" if "azure_subscription_id" in data else "gcp" if "gcp_project_id" in data else "cloud"
                ...
                if isinstance(findings_data, dict):
                    for finding_key, finding_data in findings_data.items():
                        if isinstance(finding_data, dict):
                            self._extract_finding(finding_key, finding_data, provider, aws_account_id, findings, seen)
    ...
    def _extract_finding(self, key: str, data: dict, provider: str, account_id: str, findings: list, seen: set) -> None:
        description = data.get("description", data.get("dashboard_name", key))
        severity_str = data.get("severity", data.get("level", "info")).lower()
        severity = severity_str if severity_str in ("critical", "high", "medium", "low", "info") else "info"
