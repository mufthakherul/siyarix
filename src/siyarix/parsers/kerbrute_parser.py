...
_VALID_USER_RE = re.compile(r"(?:\d{4}/\d{2}/\d{2}\s+\d{2}:\d{2}:\d{2}\s+>\s+)?\[\+\]\s+(?:VALID USER(?:NAME)?|Found|User found)[:\s]\s*(\S+)", re.IGNORECASE)
...
class KerbruteParser:
    ...
    def parse(self, output: str) -> list[dict]:
                ...
                for item in items:
                    ...
                    if user and valid in (True, "true", "valid"):
                        if user in seen_users:
                            continue
                        seen_users.add(user)
                        target = item.get("domain", item.get("host", target))  # type: ignore
                        findings.append({
        ...
        for line in output.splitlines():
            ...
            if ip_m and ("dc" in line_stripped.lower() or "domain" in line_stripped.lower() or "krb" in line_stripped.lower()):
                target = ip_m.group(1)
