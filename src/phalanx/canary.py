...
class CanaryTokenManager:
    ...
    def __init__(self, storage_dir: Path | None = None) -> None:
        self._storage_dir = storage_dir or Path.home() / ".phalanx" / "canary"
    ...
    def _generate_token_value(
        ...
        generators = {
            ...
            CanaryTokenType.DNS: lambda: (
                f"canary-{token_id}.{location.split('.', 1)[-1] if location else 'phalanx-alert.local'}"
            ),
            ...
            CanaryTokenType.DB_RECORD: lambda: f"phalanx_canary_{token_id}",
