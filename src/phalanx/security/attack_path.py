"""Phalanx Attack Path Analyzer — Identifies exploit paths from the Knowledge Graph."""
...
class AttackPathAnalyzer:
    ...
    def find_all_paths(self) -> list[AttackPath]:
        ...
        for cred in cred_nodes:
            ...
            connected_services = []
            for edge in self.graph.get_edges(source_id=cred.node_id, edge_type=EdgeType.AUTHENTICATED_BY):
    ...
    def generate_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "paths": [
                {
                    "origin": self.graph.get_node(p.origin_id).label if self.graph.get_node(p.origin_id) else p.origin_id,
                    "target": self.graph.get_node(p.target_id).label if self.graph.get_node(p.target_id) else p.target_id,
                    "severity": p.severity,
