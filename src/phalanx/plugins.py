...
_DEFAULT_ROOT = Path(
    os.getenv("PHALANX_PLUGINS_DIR", str(Path.home() / ".phalanx" / "plugins"))
)
...
class PluginManager:
        ...
        for plugin in self.list_plugins():
            ...
            try:
                module = self._load_module(
                    module_file, f"phalanx_plugin_{plugin.name}_commands"
                )
            except Exception:
    ...
    def load_parser_plugins(self) -> dict[str, Callable[[str], list[dict]]]:
        ...
        for plugin in self.list_plugins():
            ...
            try:
                module = self._load_module(
                    module_file, f"phalanx_plugin_{plugin.name}_parser"
                )
            except Exception:
                logging.getLogger(__name__).exception(
