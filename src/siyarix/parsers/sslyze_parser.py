...
class SslyzeParser:
    ...
    def parse(self, output: str) -> list[dict]:
                ...
                for sev in ("error", "warning", "info", "high"):
                    if sev in lower:
                        ...
                        findings.append({
                            ...
                            "severity": "high" if sev == "error" else "medium" if sev in ("warning", "high") else "info",
        ...
        port = server_info.get("port", server_info.get("network_configuration", {}).get("port", 443))
            ...
            for scan_type, scan_result in results.items():
                if isinstance(scan_result, dict):
                    ...
                    if isinstance(scan_result_data, dict):
                        self._parse_scan_result(scan_type, scan_result_data, hostname, port, findings, seen)

        return findings

    def _parse_scan_result(self, scan_type: str, data: dict, hostname: str, port: int, findings: list, seen: set[str]) -> None:
        if "tls_version" in scan_type or "protocol" in scan_type.lower():
            ...
            cert = data.get("certificate", data.get("certificate_chain", [{}]))[0] if isinstance(data.get("certificate_chain"), list) else data.get("certificate", {})
