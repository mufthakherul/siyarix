...
Distributed Multi-Agent Deployment — Replace the in-process AgentTeam with a Redis/RQ-backed task queue so multiple Siyarix instances can share work. Move the SQLite offline store to PostgreSQL for the server role.
...
Benchmarking & Red Team Eval Suite — Create a controlled test environment (Docker Compose with intentionally vulnerable targets like DVWA, Metasploitable) and benchmark Siyarix's detection/exploitation success rates, comparing AI-only vs. classic-only vs. hybrid modes.
