...
class OllamaClient:
    ...
    def __init__(self, host: str, model: str, temperature: float = 0.2, top_p: float = 0.95, max_tokens: int = 512):
        self.host = host
        self.model = model
...
class CodeAnalyzer:
    ...
    def extract_elements(code: str, language: str) -> List[Dict]:
        ...
        if language == 'python':
            for match in re.finditer(CodeAnalyzer.PYTHON_PATTERNS['function'], code, re.MULTILINE):
                elements.append({
                    'type': 'function',
                    'name': match.group(1),
            ...
            for match in re.finditer(CodeAnalyzer.PYTHON_PATTERNS['class'], code, re.MULTILINE):
                elements.append({
                    'type': 'class',
                    'name': match.group(1),
...
class CommentGenerator:
    ...
    def __init__(self, client: OllamaClient, style: str, output_language: str, scope: str):
        self.client = client
        self.style = style
...
Output ONLY the comment text, no code blocks or explanations. Start directly with the comment."""
    ...
    def apply_comment(self, lines: List[str], line_num: int, comment: str, language: str) -> List[str]:
        ...
        try:
            ...
            if len(original) > self.config.max_file_size:
                if self.config.verbose:
                    print(f"Skipping {filepath}: file too large ({len(original)} bytes)")
                return False, ""
            ...
            for elem in elements[:10]:  # Limit per file
                ...
                if comment:
                    lines = self.generator.apply_comment(lines, line_num, comment, language)
...
Setup:
  ...
  3. Pull model: ollama pull codellama
    ...
    parser.add_argument('--target', '-t', default='src',
                       help='Target file, directory, or "all" (default: src)')
    parser.add_argument('--file', '-f', default='',
                       help='Specific file to process')
    parser.add_argument('--style', '-s', choices=['minimal', 'standard', 'detailed', 'docstring', 'google', 'numpy'],
                       default='standard', help='Comment style (default: standard)')
    parser.add_argument('--scope', choices=['functions', 'classes', 'all'],
                       default='all', help='What to comment (default: all)')
    parser.add_argument('--model', '-m', default='codellama',
                       help='Ollama model (default: codellama)')
